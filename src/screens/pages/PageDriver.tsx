...
function PageDriver() {
  ...
  const [selectedRoute, setSelectedRoute] = React.useState<RouteData | null>(
    null,
  );
  ...
  const [devices, setDevices] = React.useState<
    Array<{
      ...
      icon_color?: string;
      icon_colors?: string;
      device_data?: { icon_color?: string };
    }>
  >([]);
  const [selectedVehicleName, setSelectedVehicleName] =
  ...
  >(undefined);
  const [selectedDriverCoord, setSelectedDriverCoord] = React.useState<
  ...
  React.useEffect(() => {
    loadingRef.current = loadingDevices;
  }, [loadingDevices]);
  ...
  React.useEffect(() => {
    const init = async () => {
      ...
      Geolocation.getCurrentPosition(
        ...
        _err => {
          console.log('PageDriver initialRegion error', _err);
          setInitialRegion({
            latitude: -34.6037,
            longitude: -58.3816,
            latitudeDelta: 0.02,
  ...
  React.useEffect(() => {
    const loadRoutes = async () => {
      try {
        ...
        const filteredRaw =
          cid != null
            ? Array.isArray(rawList)
              ? rawList.filter(
                  (r: any) => Number(r?.company_id) === Number(cid),
                )
              : []
            : Array.isArray(rawList)
            ? rawList
            : [];
        const effectiveRaw =
          Array.isArray(filteredRaw) && filteredRaw.length > 0
            ? filteredRaw
            : Array.isArray(rawList)
            ? rawList
            : [];
        ...
        const mapped: RouteData[] = (
          Array.isArray(effectiveRaw) ? effectiveRaw : []
        ).map((r: any) => {
          const secs = Number(
            r?.estimated_travel_time_seconds ??
              r?.estimated_driving_time_seconds ??
              0,
          );
          ...
          const item: RouteData = {
            ...
            type: (() => {
              const raw = String(
                r?.type ?? r?.route_type ?? r?.Direction ?? r?.direction ?? '',
              ).toLowerCase();
          ...
          console.log('PageDriver route item', {
            id: item.id,
            name: item.name,
            time: item.time,
        ...
        const finalList =
          mapped.length > 0 ? mapped : (routesFallback as RouteData[]);
  ...
  const loadDevices = React.useCallback(async (p?: number) => {
    try {
      ...
      const masked = token
        ? `${String(token).slice(0, 6)}...${String(token).slice(-4)}`
        : null;
      const url = '/devices/v1/devices';
      const baseURL = (api.defaults as any)?.baseURL;
      const headers = token ? { Authorization: `Bearer ${token}` } : undefined;
      console.log('PageDriver devices GET', {
        baseURL,
        url,
        params: { page: pageToLoad },
        headers: headers ? { Authorization: `Bearer ${masked}` } : null,
      });
      const res = await api.get(url, {
        headers,
        params: { page: pageToLoad },
      } as any);
      console.log('PageDriver devices response', {
        status: res.status,
      ...
      console.log('PageDriver devices parsed', {
        count: arr.length,
        first: arr[0]?.name,
        page: pageToLoad,
      });
      setDevices(prev =>
        pageToLoad === 1 ? (arr as any) : ([...prev, ...arr] as any),
      );
      ...
      try {
        ...
        }/devices/v1/devices?page=${
          typeof p === 'number' ? p : pageRef.current
        }`;
        const res2 = await fetch(urlFull, {
          headers: token ? { Authorization: `Bearer ${token}` } : {},
        });
        const text = await res2.text();
        console.log('PageDriver devices fetch fallback', {
          status: res2.status,
          ok: res2.ok,
        ...
        console.log(
          'PageDriver devices fetch fallback error',
          e2?.message || e2,
        );
      }
    } finally {
  ...
  const BottomContent: React.FC<BottomArgs> = ({
    collapsed,
    toggle,
    onRouteSelect,
    onModeChange,
    ...
    <MenuRoutes collapsed={collapsed}>
      {isDetails && selectedRoute ? (
        <DetailRoutes
          ...
          onRouteSelect={stops => {
            console.log(
              'PageDriver DetailRoutes onRouteSelect:',
              stops?.length || 0,
            );
            onRouteSelect?.(stops);
          }}
        ...
        <RoutesMenu
          ...
          onRouteSelect={stops => {
            console.log(
              'PageDriver onRouteSelect called with stops:',
              stops?.length || 0,
            );
            onRouteSelect?.(stops);
          }}
          ...
          onSelectRoute={route => {
            console.log(
              'PageDriver onSelectRoute called:',
              route?.id,
              route?.name,
            );
          ...
          <View style={[styles.overlayCard, { top: insets.top + 90 }]}>
            ...
            <View style={styles.plateBox}>
              <Text style={styles.plateText}>
                {selectedVehicleName || 'NZF-056'}
              </Text>
            </View>
            <TouchableOpacity
              style={styles.smallButton}
              onPress={() => setVehiclesOpen(v => !v)}
            >
    ...
    <View style={StyleSheet.absoluteFill}>
      ...
      <VehicleListModal
        ...
        onSelect={item => {
          ...
          setSelectedVehicleColor(
            String(
              (item as any)?.icon_colors ||
                (item as any)?.icon_color ||
                (item as any)?.device_data?.icon_color ||
                '#000',
            ),
          );
        ...
        hasMore={hasMore}
        onLoadMore={() =>
          hasMore && !loadingDevices
            ? loadDevices(pageRef.current + 1)
            : undefined
        }
      />
    </View>
